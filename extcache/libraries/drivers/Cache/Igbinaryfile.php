...
/**
 * File-based Cache driver with igbinary serialize.
 *
 * $Id: Igbinaryfile.php 4046 2015-11-13 19:23:29Z pokimon $
 *
 * @package  Cache::IgbinaryFile
 * @version  2.0
 * @author   Anton Paramonov <paramonovav@gmail.com>
 * @license  https://kohanaframework.org/license
 */
class Cache_IgbinaryFile_Driver extends Cache_File_Driver {
	...
	public function __construct($directory)
	{
		...
		{
			throw new Kohana_Exception('cache.extension_not_loaded', 'igbinary');
		}
	...
	public function exists($id, $tag = FALSE)
	{
		...
		{
			// Find all the files
			return glob($this->directory.'*~*~*.igbinary');
		}
		elseif ($tag === TRUE)
		{
			// Find all the files that have the tag name
			$paths = glob($this->directory.'*~*'.$id.'*~*.igbinary');
		...
		else
		{
			...
			return glob($this->directory.$id.'~*.igbinary');
		}
	}
	...
	public function set($id, $data, array $tags = NULL, $lifetime)
	{
		...
		return (bool) file_put_contents($this->directory.$id.'~'.$tags.'~'.$lifetime.'.igbinary', igbinary_serialize($data));
	}
