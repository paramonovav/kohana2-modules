...
/**
 * File-based Cache driver with json serialize.
 *
 * $Id: Jsonfile.php 4046 2015-11-13 19:23:29Z pokimon $
 *
 * @package  Cache::JsonFile
 * @version  2.0
 * @author   Anton Paramonov <paramonovav@gmail.com>
 * @license  https://kohanaframework.org/license
 */
class Cache_JsonFile_Driver extends Cache_File_Driver {
	...
	public function __construct($directory)
	{
		...
		{
			throw new Kohana_Exception('cache.extension_not_loaded', 'json');
		}
	...
	public function exists($id, $tag = FALSE)
	{
		...
		elseif ($tag === TRUE)
		{
			...
			$paths = glob($this->directory.'*~*'.$id.'*~*.json');
		...
		else
		{
			...
			return glob($this->directory.$id.'~*.json');
		}
	}
	...
	public function set($id, $data, array $tags = NULL, $lifetime)
	{
		...
		return (bool) file_put_contents($this->directory.$id.'~'.$tags.'~'.$lifetime.'.json', json_encode($data));
	}
